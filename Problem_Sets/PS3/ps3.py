...
def get_word_score(word, n):
    ...
    for key, value in freq.items():
        points_sum += SCRABBLE_LETTER_VALUES[key]*value
    penalty = 7*len(word) - 3*(n - len(word))
    second_points = max(penalty, 1)
    total = points_sum*second_points
    return total
#
...
def substitute_hand(hand, letter):
    ...
    if letter in hand.keys():
        ...
        new_key = random.choice(characters)
        new_hand[new_key] = new_hand.get(new_key, 0) + 1
        return new_hand
    else: 
        return hand
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(num_hands):        
        if subs_num == 0:
            subsyn = input('Would you like to substitute a letter? ')
            subsyn = subsyn.lower()
            subs_num += 1
            if subsyn == 'yes':
        ...
        else: score_hand = play_hand(hand, word_list)
        
        if replay_num == 0:
            replay_num += 1
            replayyn = input('Would you like to replay the hand? ')
            replayyn = replayyn.lower()
            if replayyn == 'yes':
                score_hand2 = play_hand(hand, word_list)
                score_hand = max(score_hand, score_hand2)
